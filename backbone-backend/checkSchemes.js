...
const checkAndSeedSchemes = async () => {
  try {
    await mongoose.connect(process.env.MONGO_URI);
    console.log('✅ Connected to MongoDB');

    const count = await Scheme.countDocuments();
    console.log(`📊 Current schemes in database: ${count}`);

    if (count === 0) {
      console.log('⚠️  No schemes found. Seeding initial schemes...');
      ...
      console.log('✅ Seeded 3 initial schemes');
    } else {
      console.log('✅ Schemes already exist in database');
      const schemes = await Scheme.find().select('name category');
      schemes.forEach(s => console.log(`  - ${s.name} (${s.category})`));
    }
